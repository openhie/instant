...
This package sets up the iHRIS and GOFR applications which act as a Provider and Facilty registry respectively. They are able to be queried for facility and practitioner information using the mCSD IHE profile. It also sets up a mediator that synchronises (using mCSD) practitioner and facility data with the central FHIR store that is provided by the core package. This allows the user of the HIE to query this interlinked data from a single location to answer questions such as the following scenario:
...
This package aim to enact [this OpenHIE workflow](https://wiki.ohie.org/pages/viewpage.action?pageId=22053221) using the newer mCSD IHE profile.
