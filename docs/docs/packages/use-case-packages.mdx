...
The goal is to allow for the collection of data from different data sources, by using an industry standard for the health information exchange and making the data availabe for analysis and reporing using the DHIS2 Covid19 dashboards.
...
The goal is to allow for the collection of data from different data sources, by using an industry standard for the health information exchange. Patient identities are uniquely maintained by making use of a client registry which matches patient records based on demographic characteristics.
