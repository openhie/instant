...
We supplied Docker Compose files for the setup and configuration of these application. We chose to split the `docker-compose.*.yml` files into three:
...
Select a deployment target below to follow the getting started steps in setting up this package.
