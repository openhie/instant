...
3. A `meta.json` file that holds metadata about the package
...
* `up` - start all the applications in this package and performs any necessary pre-processing of the infrastructure files
...
TBD, this part is still in development and will be updated once it is implemented.
